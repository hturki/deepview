...
class ModelDeepViewBlock(torch.nn.Module):
    ...
    def __init__(self, nchan=int(os.environ['NCHAN']), iteration_num=0, is_final_iteration=False, plot_mem=False):
        ...
        cnn4_extra_in = 0 if iteration_num == 0 else (self.nchan * 8 if iteration_num == 1 else self.nchan)

        cnn4_out_dim = 4 if is_final_iteration else nchan
    ...
    def forward(self, inp):
        ...
        if self.iteration_num > 0:
            ...
            pre_in = inp['mpi'].contiguous().view(depths_batches, self.nchan, height, width)
            if self.iteration_num == 1:
                pre_out = checkpoint_sequential(self.pre_cnn, segments=2,
                                                input=pre_in)  # pre_out: [depths_batches, channels=16, height//2, width//2]
            else:
                pre_out = pre_in
        ...
        maxk0 = cnn1_out.contiguous().view(depths_batches, views, channels, height, width).max(dim=1)[0] \
            .unsqueeze(1).expand(-1, views, -1, -1, -1).contiguous().view(depths_batches * views, channels, height,
                                                                          width)
